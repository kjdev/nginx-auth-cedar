...
curl -sfL -o nginx-${nginx_version}.tar.gz https://nginx.org/download/nginx-${nginx_version}.tar.gz
...
COPY config /build/
COPY src/ /build/src/
COPY nxe-cedar/ /build/nxe-cedar/
...
./configure \
  ...
  --add-dynamic-module=..
make modules
cp objs/ngx_http_auth_cedar_module.so /usr/lib/nginx/modules/
EOS
...
RUN --mount=type=cache,target=/var/cache/apk sh -ex <<'EOS'
apk upgrade
# load module: ngx_http_auth_cedar_module.so
sed -i '/events {/i load_module "/usr/lib/nginx/modules/ngx_http_auth_cedar_module.so";' /etc/nginx/nginx.conf
EOS

COPY --from=builder /usr/lib/nginx/modules/ngx_http_auth_cedar_module.so /usr/lib/nginx/modules/ngx_http_auth_cedar_module.so
